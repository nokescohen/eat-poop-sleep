...
Event types: Sleep, Sleep start, Sleep end, Breastfeed, Breastfeed start, Breastfeed end, Bottle, Bottle Feed, Poop, Pee, Antibiotic, Wound Clean, Vit D, Pump, Freeze, H2O`;
  ...
  const eventTypeMap = {
    ...
    'sleep end': 'sleep_end',
    'sleep': 'sleep_start', // Default sleep to start
    'breastfeed start': 'breast_start',
    'breastfeed end': 'breast_end',
    'breast start': 'breast_start',
    'breast end': 'breast_end',
    'breastfeed': 'breast_start', // Default breastfeed to start
    'breast': 'breast_start'
  };
  ...
  for(let i = 0; i < lines.length; i++){
    const line = lines[i];
    try {
      // Parse format: MM/DD/YYYY HH:MM EventType [Amount]
      // Match date (M/D/YYYY or MM/DD/YYYY), time (H:MM or HH:MM), and rest
      const match = line.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})\s+(\d{1,2}):(\d{2})\s+(.+)$/i);
      if(!match){
        errors.push(`Line ${i+1}: Invalid format - "${line}"`);
        continue;
      }
      
      const [, month, day, year, hour, minute, rest] = match;
      const date = new Date(parseInt(year), parseInt(month) - 1, parseInt(day), parseInt(hour), parseInt(minute));
      
      if(isNaN(date.getTime())){
        errors.push(`Line ${i+1}: Invalid date/time - "${line}"`);
        continue;
      }
      
      if(date > now){
        errors.push(`Line ${i+1}: Date/time is in the future - "${line}"`);
        continue;
      }
      
      // Parse event type and amount
      ...
      for(const [key, value] of Object.entries(eventTypeMap)){
        if(restLower.startsWith(key)){
          ...
          const remaining = rest.substring(key.length).trim();
          if(remaining){
            // Try to extract number (e.g., "2oz", "2 oz", "2.5oz", "3.5 oz")
            const amountMatch = remaining.match(/^(\d+\.?\d*)\s*oz?$/i);
            if(amountMatch){
      ...
      if(!eventType){
        errors.push(`Line ${i+1}: Unknown event type - "${line}"`);
        continue;
      }
      
      // Create event
      const eventData = {};
      if(amount !== null && (eventType === 'feed' || eventType === 'pump' || eventType === 'freeze' || eventType === 'h2o')){
      ...
      errors.push(`Line ${i+1}: Error parsing - "${line}" (${error.message})`);
    }
  }
